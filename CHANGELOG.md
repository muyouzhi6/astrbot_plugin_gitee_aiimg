...
- 修复聊天工具通过形象库生成 Bot 自拍时漏掉自定义自拍前缀, 支持 self、人物 ID 和名称; 多人合影中的身份与穿搭描述仅约束 Bot.
- 批量人物自拍在任务受理时固定自定义要求, 每张图在镜头规划后重新附加, 避免规划模型遗漏或后续配置修改影响已受理任务.
- 为 ztyunjuan.com 的 Happy Horse 视频模型使用 JSON 首帧请求, 校验时长和重复参考输入, 保留其他 OpenAI 视频渠道的文件上传协议.
- 补充同步单图、后台单图、批量人物自拍及 Happy Horse 请求协议回归测试和使用说明.

## [v5.7.0] - 2026-09-16

- 新增 Agnes Video 2.5 Flash 模板, 使用 JSON 创建任务及 video_id/model_name 轮询, 兼容文档 metadata.url 与实测顶层 url 结果格式.
- 视频工具增加自拍转视频模式, 先固定 Bot 身份和日程穿搭生成底图, 再用该图片生成视频; 自拍失败不会继续文生视频.
- 创建与查询共享持久化限速, 默认请求间隔 61 秒, 遵守 RPM 1; 校验 720P、4 至 12 秒、模式与参考数量.
- 创建结果不明和轮询中断不会重建任务或触发下一渠道, 避免重复生成. LLM 视频工具说明明确区分图片与视频请求.
- 移除 3365、SD2.0 专用模板及注册, 保留其他视频渠道和历史媒体.
- 补充 Agnes 平台注册、限时免费说明、模型与 JSON 配置示例, 区分自拍视频工具模式和 provider 模式; 整理 WebUI、批量创作、画布、节点工作流、形象库及画廊的操作文档.
- 重整 README 为介绍、特色、服务商推荐、分类出图展示和教程导航; 恢复 Gitee 历史申请截图, 补充美年达双协议配置, 收录 10 张作者自拍展示, 将详细操作拆分到 docs/ 并前置 QQ 群入口.

## [v5.6.1] - 2026-09-15

- 修复普通 HTTP 管理页面中创建服务商、工作流、画布图层和生成请求依赖安全上下文 UUID 接口的问题.
- 服务商分为模型连接与回退链路, 提供可搜索的模型选择器、就近保存、明确的密钥保留提示、JSON 错误定位和手机独立编辑页.
- 节点增加步骤导航、编辑按钮、待配置提示及输入来源选择; 手机优先显示参数编辑, 运行结果直接展示.
- 修复保存期间继续编辑导致新内容丢失, 避免旧任务完成状态显示在已修改的节点配置上; 增加前端回归测试.
- 修复自由画布中发起镜头规划后看不到清单的问题, 规划与生成自动切换到结果视图.

## [v5.6.0] - 2026-09-15

- 新增可保存和执行的节点工作流: 提示词、参考图片、出镜人物、镜头规划、图片生成与结果输出, 支持拖放、类型连线、移动端来源选择、模板及 JSON 导入导出.
- 执行前校验循环、缺失输入和断开节点; 运行时固定工作流与人物参考, 复用现有任务调度器, 记录逐节点状态, 支持停止及刷新恢复, 失败保留已完成图片并停止后续节点.
- 画廊支持单张和批量删除、收藏夹及收藏防删; 人物参考、工作流参考和任务使用中的图片受到保护.
- 支持最大图片保留数量, 默认不限. 超限时清理最旧的可删除图片, 新图片至少保留 5 分钟, 收藏和引用图片不因超限而删除.
- 删除同步清理画布引用并更新版本; 共享原图在最后一条有效记录删除后才释放, 已删除结果不再重新加入画布.

## [v5.5.0] - 2026-09-15

- 重构工作区为创作与选片布局, 明确提供自由生成、图片编辑、成片变体、严格仿拍和换装拍摄入口, 保留可切换的自由画布.
- 支持每组 1 至 12 张图片, 整组预留容量, 逐图展示状态, 部分失败保留成功结果, 失败镜头单独重试并复用原始输入.
- 变体、仿拍与换装使用可选择的 AstrBot 视觉对话模型规划镜头, 支持编辑提示词和勾选镜头后再生成. 规划过程支持离开、刷新和重载中断记录.
- 有明确人物身份时, 工作流参考图只用于视觉规划, 不传给最终生图模型; 单独绑定人物身份、目标服装与日程默认值.
- 保存工作区的创作方式、模型、数量、参考图和镜头草稿, 成片直接提供变体、仿拍、编辑和下载操作. 优化桌面与手机布局.
- 请求去重标识随工作区保存在服务器, 修复 AstrBot 隔离 iframe 禁止访问 sessionStorage 时无法提交生成的问题.

## [v5.4.0] - 2026-09-15

- 新增 AstrBot 插件 Pages 映像工作台: 服务商模型列表获取, 可拖动回退链路, 私有画廊, 多人物形象库和持久化画布.
- 支持每个人物多套身份参考, 按当前会话切换 Bot 形象; 增加 /形象, /换形象, /人物 中文指令.
- 单图及批量工具支持显式出镜人物与逐人穿搭; Bot 默认读取 life_scheduler 缓存, 他人穿搭独立处理. 拒绝歧义身份, 未授权引用和不同人物共用同一身份图.
- 画廊独立保存原图和生成提示词, 支持预览, 下载和继续编辑. 导入旧缓存时保留缺少提示词的真实状态.
- 画布支持参考图选用, 拖动缩放, 图层复制/旋转/排序, 撤销重做, 非破坏性裁剪和导出; 适配移动端触摸操作.
- 工作台任务共用现有调度器, 保留去重, 取消和重载中断记录; 隔离排队任务的上下文, 避免并发归档错绑.
- 配置保存保留密钥, 阻止地址改变时复用隐藏密钥, 检查版本冲突, 保存前备份. 使用 AstrBot 4.28 的插件 Web API, 最低版本调整至 4.28.0.

## [v5.3.1] - 2026-09-14

- 服务商折叠列表隐藏冗长说明, 避免说明挤占删除按钮; 添加菜单优先展示 OpenAI Images, Gemini 原生和 OpenAI Chat 出图.
- 所有服务商统一提供可编辑的额外请求体, 支持 JSON 对象及嵌套参数; OpenAI Images 提供可选 Quality 参数入口.
- 补齐 Gemini 原生, Flow2API, Gitee, Vertex, 即梦及视频后端的额外参数传递; Gemini 嵌套合并保留默认比例和分辨率.
- 增加模板编辑契约, 参数路由和实际请求体回归测试.

## [v5.3.0] - 2026-09-07

- GPT Image 系列 Images API 改为按顺序上传多张独立参考文件，保留旧模型单图兼容行为；GPT Image 尺寸错误不再自动降低4K请求。
- 支持 ContextAware 历史参考的后台批量改图与混合参考自拍；整组复用接单时的参考字节、哈希和用途，不随群聊后续消息更换参考。
- 每张成功发送的批量结果使用独立子任务 ID 登记回 ContextAware，保留父参考与请求者；失败或发送结果未知的图片不登记为已投递结果。
- 增强 `object` 角色对具体动物/物体的外观约束，保留夸张脸型、比例、材质与画风，避免默认写实风格覆盖参考主体。
- 新增按需视觉特征提取：只读取选中 object 的预览，后台接单后执行，整批一次，最多45秒；能力不可用、超时或解析失败时保留原始参考继续生成，并记录状态。
- 精简混合参考自拍的默认提示，保留用户明确要求的优先级；Gemini native 按图片编号传递，去掉默认要求重新想象输入图的措辞。
- 更新单图/批量工具说明，数量请求应选择批量并保留引用；历史参考批量在后台模式不可用时明确失败。
- 新增批量引用生命周期、原消息释放后输入保持、部分发送失败、视觉预览与索引、超时取消和 Gemini 多图请求测试。

## [v5.2.0] - 2026-09-07

- 单图 aiimg_generate 增加 ContextAware 图片 ID 与参考角色，支持历史改图、固定身份加猫/服装等混合参考自拍。
- 显式引用失败或模式冲突时终止，不降级无参考生图；同一事件并发调用保护，任务输入与父引用摘要一致。
- 后台接单保存引用输入、哈希和角色；成功发送的单图结果登记回 ContextAware，便于原请求者继续编辑，登记失败不会重发生图。
- 多参考角色任务仅允许有序多图后端，禁止静默首图/拼图降级；保持既有4K/比例规则。
- 批量历史引用明确拒绝；旧模式无引用调用不变。
- 新增跨插件输入校验、任务副本、自拍顺序、结果交付登记、并发和后端能力测试。

## [v5.1.29] - 2026-08-25

### 修复

- 3365 鉴权 `/content` 改为不带 Range 的单请求流式下载，避免探测加并发分块耗尽限次媒体 URL 后出现中途 `404`，也避开超大 Range 请求卡满下载超时。
- 3365 创建任务仅对明确返回的 `408/409/425/429/5xx` 做有限重试；连接超时等结果不明的异常仍不重试，避免重复任务与重复扣费。

### 测试

- 新增 3365 单请求无 Range 媒体下载、瞬时 `502` 创建重试及不确定超时不重试的回归测试。

## [v5.1.28] - 2026-08-25

### 修复

- 视频下载连接超时不再写死为 10 秒，改为随 `download_timeout_seconds` 放宽且最多 120 秒，适配京东云到 3365 的慢 TLS 链路。
- 分段下载与普通续传现在会重试 `ConnectTimeout` 等 `httpx.TimeoutException`，避免单个分块重连超时直接终止整段视频。

### 测试

- 新增慢连接超时配置与分段下载连接超时重试回归测试。

## [v5.1.27] - 2026-08-25

### 修复

- 3365 视频创建请求的连接超时也改为读取 `request_timeout_seconds`，避免京东云连接上游超过旧的 10 秒硬限制时提前失败并错误切换到备用渠道。

### 测试

- 扩展 3365 请求超时回归测试，覆盖 connect、read 与 write 三个阶段。

## [v5.1.26] - 2026-08-25

### 修复

- 3365 图生视频创建请求不再使用写死的 10 秒上传超时，改为读取 `request_timeout_seconds`（默认 120 秒），避免 base64 参考图在慢链路上传时被提前中断。
- 视频图片快照增加 event/raw 候选数量、最终字节数与 MIME 日志，便于确认 ContextAware 临时图片是否已成功固化或经 OneBot 回源。

### 测试

- 新增 3365 图片上传请求超时配置回归测试。
- 使用 `grok-imagine-video-1.5` 完成真实文生与图生任务；图生结果已下载为可解析的 H.264/AAC MP4。

## [v5.1.25] - 2026-08-25

### 修复

- 美年达视频任务在上游明确返回 token pool 暂无可用 token 时，按配置次数有限重建任务；余额不足、无可用 channel 和超时不会重复提交。

### 测试

- 新增 token pool 异步失败重建与非 token 故障不重试的回归测试。

## [v5.1.24] - 2026-08-25

### 修复

- URL 和本地文件都发送失败时不再发送纯文本 URL 并标记视频成功，改为抛出错误触发 provider 链路兜底，避免把“QQ 没收到视频”伪装成完成。

### 测试

- 新增 URL 与文件发送同时失败时的失败回归测试。
