...
通过聊天工具的 `character_ids` 调用形象库中的 Bot 时, 自定义前缀同样生效, 支持 `self`、Bot 人物 ID 和名称. 单图和批量都会将它加入最终生图提示词; 批量任务在受理时保存这段要求, 不依赖镜头规划模型复述. 多人合影时, 其中的外貌、穿搭和身份描述只约束 Bot; 只拍其他人物时不加入 Bot 自拍前缀.
