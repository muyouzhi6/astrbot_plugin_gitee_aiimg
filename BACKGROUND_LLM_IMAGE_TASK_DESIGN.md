...
截至 `2026-07-30`，本文首版范围已经在插件仓库本地实现：新增 SQLite single-owner 任务管理器、single/batch parent-child、容量背压、公平调度、输入固化、投递 receipt、通知 outbox、ContextAware gate、reset/new send gate 和 pipeline synthetic completion；全量自动化回归为 `148 passed`，命令为 `PYTHONPATH=.:tests uvx --from pytest --with pytest-asyncio --with pillow --with httpx --with aiohttp --with aiofiles --with openai --with curl-cffi pytest -q`，Python 编译与定向 Ruff 检查通过。后台功能仍默认关闭，尚未在京东云完成真实 QQ/微信断线、热重载、AstrBot 重启和连续 `72` 小时灰度，因此不能把“本地实现与测试通过”表述为“线上长期运行已经验证”。
