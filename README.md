...
**让 Bot 自己规划一组不同镜头.** 例如说“给我拍 4 张你在窗边的照片, 同一套衣服, 每张换个姿势和表情”, LLM 会一次调用批量工具, 先规划不同的姿势、表情、机位和构图, 再逐张生成. 整组共用同一套 Bot 身份, 不会把同一张图机械重复 4 次.

未指定数量时默认 4 张, 允许 `1-32` 张. 开启后台生图后, Bot 先接单并继续聊天, 图片完成后再逐张发送.
...
| **10 · 批量、变体、仿拍与换装** | 从一张成片继续规划多组镜头, 编辑每张提示词, 勾选后批量生成; 失败项单独重试, 已成功图片不重跑.                                                                           |
