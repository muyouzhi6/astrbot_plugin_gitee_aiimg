...
从模板开始后, 点击顶部步骤或节点里的编辑按钮填写内容, 再点下一步设置模型和数量. 手机将参数编辑放在画布之前. 待设置项会标在步骤上, 点击运行时会先定位缺失项. 点击保存保留修改, 运行结果和历史记录显示在画布下方.

打开工作流, 从自由生图、成片变体或人物仿拍模板开始. 拖放节点, 点击输出圆点再点击同类型输入圆点连线; 手机端可直接在节点设置中选择输入来源. 可自行连接提示词、图片、人物、镜头规划、生成和结果输出, 为模型节点分别配置模型. 保存后运行, 规划结果会自动交给下游生成节点. 支持 JSON 导入导出, 只执行工作台内置节点类型. 这不是 ComfyUI 工作流导入接口.

工作流按依赖顺序执行, 循环和缺少输入会在发起生成前阻止. 人物参考在运行开始时固定. 节点失败后停止后续节点, 已完成图片仍保留; 页面刷新只恢复状态, 不重新发起付费生成.

画廊每张图片可以收藏或删除, 批量管理可多选、收藏与删除. 收藏图片必须先取消收藏才能删除. 人物身份参考、保存的工作流参考和正在使用的任务图片会跳过删除. 删除普通图片会同步移除工作区图层, 原图文件不可恢复.

存储设置可指定最大保留图片数量, 0 为不限, 初始默认不限. 清理按图片时间从旧到新, 每分钟检查一次, 设置上限后立即应用; 新图片至少保留 5 分钟. 收藏与引用受保护, 因此受保护数量超过上限时实际数量可以高于上限. 设置仅管理工作台资产, 不修改插件原有临时缓存策略.

在 AstrBot 插件详情中打开 **映像工作台**, 无需另开端口或设置另一套密码.

工作区默认使用选片视图. 普通生图填写描述并选择 1 至 12 张即可; 每张成片下方可以直接选择变体、仿拍或编辑. 需要移动和拼排图片时, 切换到自由画布.

变体、严格仿拍和换装拍摄: 选择参考图, 选择 AstrBot 中已配置且支持看图的对话模型, 点击规划镜头. 检查并修改每张提示词, 勾选需要的镜头后生成. 仿拍和换装还需要选择出镜人物, 多人时指定参考图对应人物. 有人物身份时, 最终图像模型只接收身份参考, 工作流目标图经过视觉规划转换为文字, 避免目标照片的人脸污染身份.

批量结果按组展示, 成功图片立即进入画廊. 失败项可以单独重试, 使用原任务保存的参考图和提示词; 已成功的图片不重跑. 规划失败不会发起生图, 重载中断的任务不会自动重新扣费生成.

- 服务商: 在模型连接中添加接口, 填写地址和密钥, 获取模型后搜索并选择, 点击保存并生效. 在回退链路页拖动手柄排序, 点击连接名称设置单项画幅, 手机也可使用上下移动按钮. 已保存密钥留空保留, 换地址时需要重新填写密钥. 配置变更先备份再生效.
