...
[新手图解](docs/quickstart.md) · [特色功能](#特色功能) · [推荐服务商](#推荐服务商) · [出图展示](#出图展示) · [申请与配置教程](#申请与配置教程) · [更新日志](CHANGELOG.md)
...
>
> 支持 `QQ / aiocqhttp` 和个人微信 `weixin_oc`. 需要 **AstrBot >=4.28.0 且 <5**.

## 特色功能

### 01 · 预设 Bot 形象自拍

**本插件的首创设计之一.** 预先保存 Bot 的身份参考图, 说一句“拍张你在窗边的照片”, 就能带着固定形象拍摄新照片. 支持多套形象, 用 `/换形象 日常` 切换当前会话的 Bot 造型.

[看实际自拍成片](#美年达-gemini-自拍) · [设置自拍与形象库](docs/studio.md#形象库与日程联动)

### 02 · LLM 后台生图, 不阻塞主对话

**本插件的另一项首创设计.** LLM 接单后立即回到对话, 单图、自拍、改图和批量任务在后台执行. 等待期间可以继续聊天, Bot 知道任务进度, 完成后再按当前人格回来回应.

[开启后台生图](docs/background-and-context.md#llm-后台生图)

### 03 · LLM 批量出图, 一次拍一组

**让 Bot 自己规划一组不同镜头.** 说“给我拍 4 张你在窗边的照片, 同一套衣服, 每张换个姿势和表情”, LLM 会一次调用批量工具, 先规划不同镜头, 再共用身份参考逐张生成.

未指定数量时默认 4 张; 单次上限默认 8 张, 可设置为 `1-32`. 开启后台生图后, 等照片时可以继续聊天.

[按美年达示例配置](docs/quickstart.md#让-bot-一次拍一组照片) · [批量工具参数](docs/configuration.md#批量出图)

### 让创作连起来的更多能力

| 特色                          | 可以做什么                                                                                                                                                              |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **04 · Fallback 回退链路**    | 文生图、改图、自拍和视频分别配置主用与备用服务商. 按顺序处理可回退的失败, 不必每次手动切换.                                                                             |
| **05 · WebUI / 画布 / 画廊**  | 获取模型列表、拖动链路、可视化编辑配置; 在画廊看提示词和大图, 在自由画布排版, 用节点工作流保存创作流程.                                                                 |
| **06 · 预设提示词**           | 保存常用文生图、改图和视频提示词, 用“预设名 + 本次要求”调用; 改图预设还可注册成独立中文指令.                                                                            |
| **07 · 他人形象与 Bot 合影**  | 为自己或其他人物建档, 将聊天里的“我”绑定到对应形象. 可以说“我们合照”, 也可以只给“穿西装的我”拍照. 人物身份与穿搭分别约束.                                               |
| **08 · 每天不同的穿搭与场景** | 联动 [日程与穿搭插件 life_scheduler](https://github.com/muyouzhi6/astrbot_plugin_life_scheduler), 让 Bot 默认穿当天的衣服、置身当天的生活场景. 用户明确要求优先.        |
| **09 · LLM 拍视频**           | “你拍个你跳舞的视频我看看”会先按 Bot 形象生成自拍底图, 再转成视频. 普通图片也可以直接做动画, 没有图片时可文生视频.                                                      |
| **10 · 变体、仿拍与换装**     | 从一张成片继续规划多组镜头, 编辑每张提示词, 勾选后批量生成; 失败项单独重试, 已成功图片不重跑.                                                                           |
| **11 · 历史图片与精细控制**   | 联动 [ContextAware](https://github.com/muyouzhi6/astrbot_plugin_context_aware) 引用聊天里的历史图片; 支持比例、精确尺寸、1K/2K/4K、多 Key 轮询、质量参数和无损图片保存. |

## 推荐服务商

以下为作者常用的三个服务商, 免费额度和价格以平台实时页面为准.

### Gitee AI · 免费文生图入门

**推荐模型: `z-image-turbo`**. 真人写实效果不错, 支持最高 2K. 作者使用的免费额度为 **每天 100 张**, 适合入门体验文生图.

作者实测内容限制较少, 可生成 NSFW 题材.

> **只支持文生图, 不能进行参考图自拍、改图或身份合影.**
> 只配置此模型时不要启用参考图自拍. 可以在人设中详细描述人物外貌, 让 LLM 每次带上描述生成“伪自拍”, 但无法锁定同一张脸.
...
**作者日常使用并推荐的生图站点**, 价格实惠, 出图质量高. 真人写实与 Bot 自拍推荐 **Gemini 香蕉系列**, 二次元与插画推荐 **GPT Image 2 / 2.5 系列**.

| 用途                         | 模型与接入建议                                                                                                            |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| 真人写实、Bot 自拍、日常合影 | `gemini-3.1-flash-image`, 使用 **Gemini 原生** 模板. 下方 10 张 Bot 照片均由作者通过本插件自拍模式生成.                   |
| 二次元、插画、文生图与改图   | GPT Image 全系列, 包括 `gpt-image-2`、`gpt-image-2.5-flare` 和 `gpt-image-2.5-sunburst`, **统一使用 OpenAI Images 模板**. |

教程提供 **美年达香蕉** 与 **美年达 GPT Image** 两套配置示例, 可分别加入自拍、改图和文生图链路.

[注册美年达](https://meinianda.top/sign-up?aff=Qs4O) · [查看 Bot 自拍成片](#美年达-gemini-自拍) · [原生配置图解](docs/quickstart.md) · [申请 Key 与两套配置示例](docs/providers.md#美年达)

<sub>注册链接包含作者 AFF 推荐标识.</sub>
...
<sub>限时免费仅覆盖 Agnes 视频, 自拍底图按图片服务商计费.</sub>
...
作者使用插件自拍模式生成的 10 张 Bot 照片, 点击图片查看大图.
...
[先看美年达原生配置截图](docs/quickstart.md)

| 按需阅读                                               | 内容                                                                |
| ------------------------------------------------------ | ------------------------------------------------------------------- |
| [Gitee 图文申请教程](docs/providers.md#gitee-ai)       | 申请 API Key、查看免费额度, 配置 2K 文生图与文字伪自拍              |
| [美年达申请与配置](docs/providers.md#美年达)           | 创建令牌、选择分组, 配置香蕉与 GPT Image 系列                       |
...
工作台与节点工作流用于图片创作, 视频通过指令或 LLM 工具调用. 详细能力与限制见 [工作台指南](docs/studio.md).
