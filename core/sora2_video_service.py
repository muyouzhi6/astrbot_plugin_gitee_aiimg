...
def _is_retryable_task_failure(data: Any) -> bool:
    """Retry only transient token-pool failures reported after task creation."""
    detail = _format_upstream_error(data).lower()
    return "no active tokens available in the pool" in detail or (
        "token pool" in detail and "no active" in detail
    )
        ...
        *,
        ...
        async with httpx.AsyncClient(
            ...
            if status in self._FAILED:
                detail = _format_upstream_error(data)
                if (
                    _is_retryable_task_failure(data)
                    ...
                    await asyncio.sleep(delay)
                    return await self.generate_video_url(
                        prompt,
                        image_bytes=image_bytes,
                        preset=preset,
                        _create_attempt=_create_attempt + 1,
                    )
                raise RuntimeError(f"Sora2 视频任务失败: {detail}")
